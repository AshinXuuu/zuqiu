...
function loadKey(){
  ...
  try { return fs.readFileSync(path.join(__dirname,'key.txt'),'utf8').trim(); } catch(e){ return ''; }
}
const KEY = loadKey();
if (!KEY) { console.error('缺少 API key:设 ODDS_API_KEY 环境变量,或在同目录放 key.txt'); process.exit(1); }
...
const server = http.createServer(function(req, res){
  ...
  upstreamGet(u.pathname + '?' + q.toString(), function(err, status, body, extra){
    if (!err && status === 200){
      ...
      persist();
      return send(200, body, extra, 'MISS');
    }
    /* 上游失败:有旧缓存就退回旧的(标记 STALE),没有才透传错误 */
...
server.listen(PORT, '127.0.0.1', function(){
  console.log('odds_proxy 监听 127.0.0.1:'+PORT+'  上游 '+UPSTREAM+'  key ****'+KEY.slice(-4));
});
